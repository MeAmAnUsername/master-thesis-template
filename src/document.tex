% Fix: From https://tex.stackexchange.com/a/508995/108670
\RequirePackage{snapshot}
\makeatletter
\def\snap@providesfile#1[#2]{%
  \wlog{File: #1 #2}%
  \if\expandafter\snap@graphic@test\expanded{#2}@@\@nil
    \snap@record@graphic#1\relax #2 (type ??)\@nil
  \else
    \expandafter\xdef\csname ver@#1\endcsname{#2}%
  \fi
  \endgroup
}
\makeatother

\documentclass[twoside,a4paper,11pt]{memoir}
\usepackage{document}
\usepackage{todonotes}
\setuptodonotes{inline}
\usepackage{listings}
\lstset{
basicstyle=\small\ttfamily,
columns=flexible,
breaklines=true
}

\newcommand{\inlinecode}{\texttt}
\newcommand{\filename}{\texttt}
\newcommand{\question}[1]{\todo[backgroundcolor=cyan]{#1}}
\newcommand{\todoLater}[1]{\todo[backgroundcolor=gray]{#1}}
\newcommand{\missingref}[1][add reference]{\todo[backgroundcolor=pink]{#1}}

% Add \@ _before_ the terminating period of the sentence (`UI\@. Test`)
% to allow inter-sentence spacing when the sentence ends with a capital letter.

\addbibresource{document.bib}
\addbibresource{researchr.bib}

\input{metadata}

\begin{document}

\frontmatter
\thispagestyle{empty}
\maketitle
\makeformaltitlepages{\input{000-abstract}}

\include{010-preface}

\cleardoublepage\tableofcontents
\cleardoublepage\listoffigures
\cleardoublepage\listoftables
\cleardoublepage\mainmatter{}

\listoftodos

\include{100-introduction}
\include{200-problem-analysis}
\include{300-pie-improvements}
\include{400-tiger}
\include{500-database-pipelines}
\include{600-testing-pipelines}
\include{700-related-work}
\include{800-conclusion}

\printbibliography{}

\include{acronyms}

\appendix
\def\chaptername{Appendix}
\include{A00-appendix}

\end{document}
