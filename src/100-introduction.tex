...
For smaller projects, such a pipeline can be expressed in a command line script like a bash script or a bat script, but as the project grows such build scripts often become unmaintainable.\autocites{Hatch2018}{Moodie2006}
...
\begin{lstlisting}
  // Shows explicit file dependencies
...
\end{lstlisting}
\begin{lstlisting}
  // shows task dependencies by using one task as input for the other
  func compileFile(file: path) -> string = {
    val program = read file;
