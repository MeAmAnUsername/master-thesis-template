...
Kotlin is a language that gets compiled to bytecode for the JVM, which means that it can interoperate with Java.
...
In Spoofax, we want one PIE file per language project, PIE is limitied to a single language project at a time.
...
One was already mentioned in <ref: NaBL2 limits static semantics>: there is no way to interoperate with generic classes like Supplier<T> and Result<T, E>.

The other issue stems from a property of tasks: not everything that is required to execute a task is an argument.
